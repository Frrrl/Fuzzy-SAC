...
fo = open('.//Render_File//Training_Complex_Comp_data.txt', 'w')  # 把训练中的数据写入到txt文件当中
env = env_for_comp(mode='complex_comp')   # 默认需要用到复杂的结合policy
agent = SAC(env)
...
for episode in range(config['max_episode']):
    ...
    for step in range(config['max_step']):
        ...
        if step % 10 == 0:  # 一个episode中每10步测试一下环境，一定要在RL开始更新之前就进行测试
            reward_test_list, danger_list = test_with_fixed_env(agent, mode='complex_comp')
            fo.write(str(reward_test_list)[1:-1]+'\n')
            fo.write(str(danger_list)[1:-1] + '\n')
            # fo.write(str(effect_list)[1:-1] + '\n')
    if episode % 10 == 8:  # 每10回合存一次agent参数
