...
config = {
    ...
    'buffer_maxlen': 2 ** 15,  # buffer最大的长度为32000，按照30个episode，每个episode4000个数据进行计算
    'batch_size': 2 ** 7,  # 一次采样的数据量为128
    'mid_dim': 128,  # 中间层设置为128
    ...
    'max_episode': 200,  # max episode
    'max_step': 400,  # max step
...
class SACActor(nn.Module):
    def __init__(self, state_dim, action_dim, mid_dim):
        super().__init__()
        self.net_combine = nn.Sequential(nn.Linear(state_dim, mid_dim), nn.ReLU())  # leakyrule
